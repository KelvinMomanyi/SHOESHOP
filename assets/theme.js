(() => {
  ...
  let predictiveDocumentListenerReady = false;
  ...
  const initReveal = () => {
    const items = document.querySelectorAll('.reveal-item:not([data-reveal-ready])');
  ...
  const initPredictiveSearch = () => {
    ...
    if (!predictiveDocumentListenerReady) {
      predictiveDocumentListenerReady = true;
  ...
  const initHeaderDrawers = () => {
    ...
    if (!headerDocumentListenersReady) {
      headerDocumentListenersReady = true;
      document.addEventListener('click', (event) => {
  ...
  const initRecipientForms = () => {
    document.querySelectorAll('[data-recipient-form]').forEach((root) => {
